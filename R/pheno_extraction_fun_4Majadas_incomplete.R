...
SplinePheno_extraction_incom<-function(data_ts,sitename,VI_name,do_norm,year_num){
  ...
  for(a in 1:1){
    ...
    #the difference between trs_eos50_2 and trs_eos25_2 cannot exceed 45day(1.5 months)
    if(min(as.numeric(abs(trs_eos25_value - ts_sm[pop2:length(ts_sm)])))>c(ampl2*0.1)|c(trs_eos25_2-trs_eos50_2)>45){
      trs_eos25_2<-NA
    }
  ...
  Pheno_result<-list(Pheno_sum=Pheno_sum,all_ts=all_ts,plot_metrics=plot_metrics,Hydro_year=paste0("H",year_num))
  return(Pheno_result)
}
